...
| `lib/harness.mjs` | The fake browser: loads `src/driftline.js` through Vite, provides a fake DOM, a virtual clock (`advance(ms)`) and records canvas drawing. |
| `lib/data.mjs` | Loads the data files from disk and builds a graph. |
| `visual/shots.mjs` | Launches Chrome/Edge headless, drives the running app through the DevTools protocol and saves PNGs (desktop and phone sizes). For looking at, not asserting. |
